...
import net.preparatusopos.app.domain.DirectProfileRequestResult;
...
implements Membership
{
	...
	@Override
	public String fetchExternalUID(ExternalUIDDomain type,
	...
	@Override
	public Token requestCredential(
			CredentialRequestInfo requestInfo, String tokenRef)
	throws MembershipException
	{
		...
		{
			case MAIL_ADDRESS:
			return requestCredentialMail(requestInfo, tokenRef);
			...
			for (PTOCredential cred : managedPrincipal.getCredentials())
			{
				...
				switch (cred.getType())
				{
					...
					case PTOCredential.TYPE_EXTERNAL:
						...
						ExternalUIDDomain type = ExternalUIDDomain.resolve(
								cred.getUsername());
	...
	@Override
	public CredentialRequestInfo addCredential(Token credReq)
	throws MembershipException
	{
		try
		{
	...
	@Override
	public MemberRegistered addMember(MemberInfo memberInfo,
			Token credReq)
	throws MembershipException
	{
		try
		{
			...
			MemberRegistered added = new MemberRegistered();
	...
	throws MembershipException
	{
		try
		{
			return requestProfile(getCallerPrincipal(), requestInfo, tokenRef);
		}
		catch (NoSuchMemberException exception)
		{
			throw newNoSuchExistingMemberException(exception);
	...
	throws MembershipException, NoSuchMemberException
	{
		try
		{
			...
			return accreditator.request(principal, requestInfo, tokenRef);
		}
		catch (AccreditatorException exception)
		{
			throw new MembershipException(exception.getMessage());
	...
	@Override
	public DirectProfileRequestResult requestDirectProfile(
			CredentialRequestInfo credentialInfo,
			ProfileRequestInfo profileInfo, String tokenRef)
	throws MembershipException
	{
		// TODO ...
		return null;
	...
	private ExternalUIDConnection getExternalUIDConnection(
			ExternalUIDDomain externalUIDType)
	throws MembershipException
	{
	...
	private Token requestCredentialMail(
			CredentialRequestInfo requestInfo, String tokenRef)
	throws MembershipException
	{
		try
	...
	private Token requestCredentialExternalUID(
			CredentialRequestInfo requestInfo)
	throws MembershipException
	{
		ExternalUIDDomain externalUIDType = requestInfo.getExternalUIDType();
		
		Token req = new Token(null);
	...
	private ExtractedCredential extractCredentialMail(Token request)
	throws NoSuchCredentialRequestException
	{
		try
		{
			...
			return new ExtractedCredential(requestInfo,
					credentialFact.newPassword(req.getMailAddress(), password));
		}
	...
	throws NoSuchCredentialRequestException
	{
		...
		PTOCredentialFactory credentialFact =
				PTORealmBootstrap.createCredentialFactory();
		return new ExtractedCredential(requestInfo, credentialFact.newExternal(
	...
	private static class ExtractedCredential
	{
		...
		public ExtractedCredential(CredentialRequestInfo requestInfo,
